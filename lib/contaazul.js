 ...
 *   token       : https://api-v2.contaazul.com/oauth/token   (exemplo de cURL do portal)
...
const PADRAO = {
  /* A tela de autorização fica em login.contaazul.com, com o caminho depois
     do "#"; o token sai da PRÓPRIA API (exemplo de cURL do portal:
     api-v2.contaazul.com/oauth/token). São servidores diferentes — mandar o
     código para o lugar errado falha com invalid_grant sem explicação. */
...
function enderecos() {
  ...
  return {
    ...
    token: c.tokenUrl || PADRAO.tokenUrl,
...
function erroToken(r) {
  ...
  if (r.status === 400 && /invalid_grant/i.test(d)) {
    return new Error('A Conta Azul recusou o código (invalid_grant). Na prática é quase sempre uma destas: o código passou dos 3 minutos, ou já tinha sido usado numa tentativa anterior. Clique em Conectar de novo e conclua logo em seguida — cada código serve uma vez só.'
      + (d && !/^invalid_grant$/i.test(String(d).trim()) ? ` [resposta da Conta Azul: ${String(d).slice(0, 200)}]` : ''));
  }
  if (r.status === 401 || /invalid_client/i.test(d)) {
    return new Error('A Conta Azul recusou o Client ID / Client Secret. O caminho mais rápido: no portal, gere um NOVO Client Secret, cole no campo daqui, salve e conecte de novo.');
  }
...
/** Troca o código que voltou na autorização pelos tokens de acesso.
    No formato exato do exemplo de cURL do portal: credencial no cabeçalho
    Basic e corpo enxuto, só com o essencial. */
...
async function renovar() {
  ...
  // Formato do exemplo de cURL do portal: Basic no cabeçalho, corpo enxuto.
  const r = await http.request(enderecos().token, {
    method: 'POST',
    headers: { Authorization: basic() },
    form: {
...
/**
 * Conecta a partir de um refresh token colado à mão.
 * O portal da Conta Azul entrega um no exemplo de cURL do app de
 * desenvolvimento — com ele a renovação é automática desde o início,
 * sem passar pela corrida dos 3 minutos do código de autorização.
 */
function refreshManual(token) {
  const c = config();
  c.refreshToken = String(token || '').trim();
  c.tokenManual = false;
  c.accessToken = '';
  c.expiraEm = 0;
  c.conectadoEm = new Date().toISOString();
  c.ultimoErro = '';
  db.save();
}
...
/* Identificação da conta é cortesia: o único endereço conhecido de userInfo
   é o do servidor de login antigo, e pode não valer para tokens da API v2.
   Quem chama já trata a falha — a conexão não depende disto. */
const USERINFO_URL = 'https://auth.contaazul.com/oauth2/userInfo';

async function quemSou() {
  const r = await chamar('', { base: USERINFO_URL });
  if (r.status !== 200) {
...
function status() {
  ...
  return {
    ...
    tokenUrl: c.tokenUrl || PADRAO.tokenUrl,
...
module.exports = {
  ...
  urlAutorizacao, consumirState, trocarCodigo, codigoDe, renovar, tokenValido, caminhoRetorno, tokenManual, refreshManual,
