...
function saleResult(sale) {
  ...
  const custoReal = custoBase + adicionais;
  const resultado = liquido - custoReal;
  const margem = bruto > 0 ? (resultado / bruto) * 100 : 0;
  return { bruto, taxa, liquido, custoBase, custosAdicionais: adicionais, custoReal, resultado, margem };
}
...
function projection(refDate) {
  ...
  for (const dias of PROJECTION_WINDOWS) {
    ...
    const aReceber = receivables.filter(r => r.vencimento <= limite).reduce((s, r) => s + r.valor, 0);
    const aPagar = payables.filter(p => p.vencimento <= limite).reduce((s, p) => s + p.valor, 0);
...
const DRE_MAP = {
  ...
  custos: {
    ...
    terceirizacao: 'Terceirizações', custo_producao: 'Custos de produção'
  },
  despesasOperacionais: {
    ...
    consorcio: 'Consórcio', despesa_operacional: 'Outras despesas'
  },
  despesasFinanceiras: {
...
module.exports = {
  ...
  previousFriday, generateInstallments,
